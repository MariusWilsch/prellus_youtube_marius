...
from .custom_prompts import create_master_document_for_expansion_prompt, create_simplified_fallback_prompt
...
class ChunkedProcessor:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.use_chapter_aligned_chunking = config.get("use_chapter_aligned_chunking", True)
    ...
    def process(self, transcript_text: str, ai_processor: TranscriptProcessorInterface, 
                mock_mode: bool = False, output_dir: str = None, config: Dict[str, Any] = None) -> str:
        """
        ...
        master_document = self._create_master_document(transcript_text, ai_processor, config)
        ...
        if scaling_factor > 1.0:
            ...
            logger.info(f"Using expansion chapters approach for high scaling factor ({scaling_factor:.2f}x)")
            processed_transcript = self._process_with_expansion_chapters(transcript_text, master_document, ai_processor, config)
            ...
            processed_transcript = self._process_with_chapter_aligned_chunks(transcript_text, master_document, ai_processor, config)
        else:
            # Use fixed-size chunking approach
            logger.info("Using fixed-size chunking approach")
            processed_transcript = self._process_chunks_with_continuity(transcript_text, master_document, ai_processor, config)
    ...
    def _parse_master_document(self, master_document: str) -> List[Dict[str, Any]]:
        ...
        if chapters:
            ...
            logger.info(f"Successfully extracted {len(chapters)} chapters from master document")
            logger.info(f"Chapter statistics: avg={avg_chapter_length:.0f}, min={min_chapter_length}, max={max_chapter_length} (target={target_chapter_size})")
            
            # Check if chapters are significantly different from target size
            if avg_chapter_length < target_chapter_size * 0.5 or avg_chapter_length > target_chapter_size * 2:
                logger.warning(f"Average chapter length ({avg_chapter_length:.0f}) is far from target size ({target_chapter_size})")
            
            # Log the first few chapters for debugging
            for i, chapter in enumerate(chapters[:3]):
                logger.debug(f"Chapter {chapter['number']}: {chapter['title']} (Pos: {chapter['start_pos']}-{chapter['end_pos']}, Length: {chapter['length']})")
            
            if len(chapters) > 3:
                logger.debug(f"... and {len(chapters) - 3} more chapters")
        else:
            logger.warning("No chapters found in master document using standard pattern")
            ...
            positions = [(int(match.group(1)), match.start()) for match in chapter_matches]
            ...
            if positions:
                ...
                for i, (num, pos) in enumerate(positions):
                    end_pos = positions[i+1][1] if i < len(positions)-1 else len(master_document)
                    ...
                    title = title_match.group(1).strip() if title_match else f"Unknown Chapter {num}"
                ...
                logger.warning("Fallback chapter detection also failed - no chapters found")
    ...
    def _create_master_document(self, transcript_text: str, ai_processor: TranscriptProcessorInterface, config: Dict[str, Any]) -> str:
        """
        ...
        target_chunk_size = min(max_chunk_size, math.ceil(target_length / required_chunks))
        
        # Extract structured prompt components from config
        prompt_role = config.get("ai", {}).get("prompt_role", "You are an expert writer tasked with processing transcript content.")
        ...
        if scaling_factor > 1.0:
            logger.info(f"Using expansion-specific master document for high scaling factor ({scaling_factor:.2f}x)")
            return self._create_master_document_for_expansion(transcript_text, target_length, ai_processor, config)
        ...
        # Log the plan
        logger.info(f"Creating master document with scaling factor: {scaling_factor:.2f}x")
        logger.info(f"Original length: {original_length} characters, Target length: {target_length} characters")
        logger.info(f"Required chunks: {required_chunks}, Original chunk size: {original_chunk_size}, Target chunk size: {target_chunk_size}")
        ...
        if original_length <= self.master_doc_max_size:
            ...
            logger.info(f"Creating master document in one pass ({original_length} characters)")
            ...
            logger.info(f"Master document created [OUTPUT SIZE: {len(master_document)} characters]")
            ...
            if self.output_dir:
                ...
                prompt_path = os.path.join(self.output_dir, "master_document_prompt.txt")
        ...
        logger.info(f"Creating master document in chunks (transcript size: {transcript_length} characters)")
        
        # Determine chunk size for master document
        master_doc_chunk_size = self.master_doc_max_size // 2  # Use half the max size for better processing
        ...
        logger.info(f"Split transcript into {total_chunks} chunks for master document creation")
        ...
        for i, chunk in enumerate(chunks):
            ...
            logger.info(f"Processing master document chunk {i+1}/{total_chunks} [SIZE: {chunk_length} characters]")
            
            # Calculate estimated number of chapters for this chunk
            estimated_chapters_this_part = max(1, round((chunk_length / transcript_length) * suggested_chapter_count))
            ...
            if i == 0:
                ...
                if chapter_matches:
                    try:
                        ...
                        logger.info(f"Found last chapter number from previous part: Chapter {last_chapter_num}")
                    except (ValueError, IndexError) as e:
                        logger.warning(f"Error parsing last chapter number: {e}. Using default of 0.")
                else:
                    logger.warning("No chapter numbers found in previous part. Using default of 0.")
            ...
            # Log the output size
            logger.info(f"Master document chunk {i+1} processed [OUTPUT SIZE: {len(chunk_master_doc)} characters]")
            ...
            if self.output_dir:
                ...
                # Save original chunk
                original_path = os.path.join(chunk_dir, f"original_master_chunk_{i+1}.txt")
                with open(original_path, 'w', encoding='utf-8') as f:
                    f.write(chunk)
                
                # Save processed chunk
                processed_path = os.path.join(chunk_dir, f"processed_master_chunk_{i+1}.txt")
        ...
        logger.info(f"Full master document created [OUTPUT SIZE: {len(full_master_document)} characters]")
        
        return full_master_document
    


    def _create_master_document_for_expansion(self, transcript_text, target_length, ai_processor, config):
            """
    ...
    """
                ...
                master_doc += "- Incorporate relevant quotes or perspectives from experts\n\n"
    ...
    def _process_with_expansion_chapters(self, transcript_text: str, master_document: str, ai_processor: TranscriptProcessorInterface, config: Dict[str, Any]) -> str:
            """
            ...
            logger.info(f"Processing transcript with extreme expansion ({scaling_factor:.2f}x)")
            ...
            logger.info(f"Divided original transcript ({original_length} chars) into {len(segments)} segments for expansion")
            ...
            for i, segment in enumerate(segments):
                ...
                logger.info(f"Processing expansion segment {i+1}/{len(segments)} for Chapter {chapter_num}")
                logger.info(f"Original segment size: {len(segment['text'])} chars, target: {max_chapter_size} chars")
                ...
                success = False
                retries = 0
                chapter_text = None  # Initialize to None to track if we ever get any response
                ...
                while not success and retries < self.max_retries:
                    try:
                        ...
                        logger.info(f"Generated Chapter {chapter_num}: {output_length} characters")
 
                        min_target_length = int(max_chapter_size * 0.7)
                        max_target_length = int(max_chapter_size * 1.7)
                        
                        if output_length < min_target_length:
                            logger.warning(f"Chapter {chapter_num} is too short ({output_length} vs. min {min_target_length}). Retrying.")
                        ...
                        if output_length > max_target_length:
                            logger.warning(f"Chapter {chapter_num} is too long ({output_length} vs. max {max_target_length}). Retrying.")
                ...
                if not success and chapter_text:
                    logger.warning(f"All retries failed for Chapter {chapter_num}. Using last generated output.")
                # If we have no output at all, create a placeholder
                elif not chapter_text:
                    logger.error(f"No valid output generated for Chapter {chapter_num}. Creating placeholder.")
                ...
                chunks_info.append({
                    ...
                    "expansion_ratio": output_length / len(segment['text']) if len(segment['text']) > 0 else 0,
            ...
            if self.output_dir:
                chunks_info_path = os.path.join(self.output_dir, "expansion_chunks_info.json")
                with open(chunks_info_path, 'w', encoding='utf-8') as f:
                    json.dump(chunks_info, f, indent=2)
            
            return expanded_transcript

    def _create_chapter_aligned_chunks(self, chapters: List[Dict[str, Any]], transcript_length: int = None) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"Maximum chapter chunk size: {max_chunk_size} characters, target chapter size: {target_chapter_size} characters")
            ...
            if is_first_chapter:
                ...
                if transcript_length:
                    ...
                    if not current_chunk["chapter_numbers"]:
                        logger.warning(f"Current chunk has no chapters, adding chapter {chapter_num}")
                    ...
                    est_chapter_size = min(target_chapter_size, transcript_length // len(chapters))
                    current_chunk_size = est_chapter_size * len(current_chunk["chapter_numbers"])
                    
                    # Check if adding this chapter would exceed the max chunk size
                    if current_chunk_size + est_chapter_size > max_chunk_size and len(current_chunk["chapter_numbers"]) > 0:
                    ...
                    max_chapters_per_chunk = max(1, max_chunk_size // target_chapter_size)  # Calculate based on target size
        ...
        if transcript_length:
            ...
            for i, chunk in enumerate(chunks):
                ...
                chunk["end_char"] = (i + 1) * chunk_size if i < len(chunks) - 1 else transcript_length
        ...
        for i, chunk in enumerate(chunks):
            ...
            logger.info(f"  Chunk {i+1}: {chapter_info}, Range: {chunk['start_char']}-{chunk['end_char']}")
        
        return chunks
    
    def _process_with_chapter_aligned_chunks(self, transcript_text: str, master_document: str, ai_processor: TranscriptProcessorInterface, config: Dict[str, Any]) -> str:
        """
        ...
        if not chapters:
            logger.warning("No chapters found in master document. Falling back to fixed-size chunking.")
            return self._process_chunks_with_continuity(transcript_text, master_document, ai_processor, config)
        ...
        for i, chunk in enumerate(chapter_chunks):
            ...
            if target_chunk_length > 20000:
                logger.warning(f"Target chunk length {target_chunk_length} exceeds API limit. Capping at 20000 characters.")
            ...
            if scaling_factor < 0.7:  # For significant condensation
                min_target_length = int(target_chunk_length * 0.7)  # 70% of target (more tolerant lower bound)
                max_target_length = int(target_chunk_length * 1.3)  # 130% of target (more tolerant upper bound)
            ...
            if scaling_factor > 1.5:
                ...
                scaling_instruction = f"maintain similar content depth with some minor adjustments"
            ...
            logger.info(f"Processing {'FINAL ' if is_last_chunk else ''}chunk {chunk_index}/{total_chunks} containing {chapter_info}")
            logger.info(f"Input size: {input_length} characters, Target size: {target_chunk_length} characters ({scaling_factor:.2f}x)")
            
            # Determine continuation instructions based on position
            if i == 0:
                continuation_instructions = "This is the FIRST chunk. Begin your narrative from the start."
                previous_context = ""
            else:
            ...
            while not success and retries < self.max_retries:
                try:
                    ...
                    prompt_with_context = CONTINUATION_PROMPT.format(
                        ...
                        continuation_instructions=continuation_instructions + previous_context + chapter_instructions,
                    ...
                    logger.info(f"Chunk {i+1} processed [OUTPUT SIZE: {output_length} characters]")
                    logger.info(f"Target was {target_chunk_length} characters (Range: {min_target_length}-{max_target_length})")
                    
                    # Validate output length against target
                    if output_length < min_target_length:
                        logger.warning(f"Processed chunk {i+1} is too short ({output_length} chars vs target min {min_target_length}). Retrying.")
                    ...
                    if not is_last_chunk and output_length > max_target_length:
                        logger.warning(f"Processed chunk {i+1} is too long ({output_length} chars vs target max {max_target_length}). Retrying.")
                    ...
                    chunks_info.append({
                        ...
                        "processed_length": output_length,
                        "scaling_factor": scaling_factor,
                        "actual_ratio": output_length / input_length if input_length > 0 else 0,
                        "within_target_range": min_target_length <= output_length <= max_target_length,
                        "start_char": start_char,
                        "end_char": end_char,
            ...
            if not success:
                logger.warning(f"All retries failed for chunk {i+1}. Using simplified processing.")
                ...
                try:
                    simple_prompt = create_simplified_fallback_prompt(
                        ...
                        target_length=target_chunk_length,
                        scaling_factor=scaling_factor,
                        previous_output_length=len(processed_chunk) if processed_chunk else 0,
                    ...
                    output_length = len(processed_chunk)
                    logger.info(f"Chunk {i+1} processed with simplified prompt [OUTPUT SIZE: {output_length} characters]")
                    ...
                    logger.error(f"Simplified processing failed for chunk {i+1}. Using original with note: {e}")
            ...
            if chunks_dir and processed_chunk:
                chunk_path = os.path.join(chunks_dir, f"processed_chapter_chunk_{i+1}.txt")
        ...
        if chunks_info:
            ...
            processed_total = sum(info.get("processed_length", 0) for info in chunks_info)
            
            logger.info(f"Chapter-aligned chunking processing completed:")
            logger.info(f"  - Original text: {original_total:,} characters")
            logger.info(f"  - Target text: {target_total:,} characters ({scaling_factor:.2f}x)")
            logger.info(f"  - Processed text: {processed_total:,} characters ({processed_total/original_total:.2f}x)")
            logger.info(f"  - Target ratio: {scaling_factor:.2f}x, Actual ratio: {processed_total/original_total:.2f}x")
    ...
    def _process_chunks_with_continuity(self, transcript_text: str, master_document: str, ai_processor: TranscriptProcessorInterface, config: Dict[str, Any] = None) -> str:
        """
        ...
        total_chunks = len(chunks)
        logger.info(f"Split transcript into {total_chunks} fixed-size processing chunks")
        ...
        if self.output_dir:
            ...
            for i, chunk in enumerate(chunks):
                original_chunk_path = os.path.join(chunks_dir, f"original_chunk_{i+1}.txt")
        ...
        for i, chunk in enumerate(chunks):
            ...
            if is_last_chunk:
                logger.info(f"Processing FINAL chunk {i+1}/{total_chunks} [INPUT SIZE: {chunk_length} characters]")
            else:
                logger.info(f"Processing chunk {i+1}/{total_chunks} [INPUT SIZE: {chunk_length} characters]")
            ...
            while not success and retries < self.max_retries:
                try:
                    ...
                    expected_min_length = min(self.min_output_length, max(int(chunk_length * 0.8), 1000))
                    
                    # Validate output length
                    if output_length < min_target_length:
                        logger.warning(f"Processed chunk {i+1} is too short ({output_length} chars vs expected min {min_target_length}). Retrying.")
                        retries += 1
                        continue
                    ...
                    if not is_last_chunk and output_length > max_target_length:
                        logger.warning(f"Processed chunk {i+1} is too long ({output_length} chars vs expected max {max_target_length}). Retrying.")
                        retries += 1
                        continue
                    ...
                    chunks_info.append({
                        ...
                        "processed_length": output_length,
                        "scaling_factor": scaling_factor,
                        "actual_ratio": output_length / chunk_length if chunk_length > 0 else 0,
                        "start_char": start_char,
                        "end_char": end_char,
                        "is_last_chunk": is_last_chunk
                    })
            ...
            if not success:
                ...
                try:
                    ...
                    logger.info(f"Using simplified prompt (first 200 chars): \"{simple_prompt[:200].replace('\n', ' ').strip()}...\"")
                    
                    # Save the simplified prompt if output_dir is provided
                    if chunks_dir:
                        simplified_prompt_path = os.path.join(chunks_dir, f"simplified_prompt_chunk_{i+1}.txt")
                    ...
                    logger.info(f"Chunk {i+1} processed with simplified prompt [OUTPUT SIZE: {len(processed_chunk)} characters]")
                    ...
                    chunks_info.append({
                        ...
                        "processed_length": len(processed_chunk),
                        "scaling_factor": scaling_factor,
                        "actual_ratio": len(processed_chunk) / chunk_length if chunk_length > 0 else 0,
                        "start_char": start_char,
                        "end_char": end_char,
                        "simplified": True
                    })
...
def process_large_transcript(transcript_text: str, config: Dict[str, Any], mock_mode: bool = False, output_dir: str = None, json_data: Optional[Dict[str, Any]] = None) -> str:
    """
    ...
    if json_data and "promptData" in json_data:
        ...
        config["ai"]["prompt_additional_instructions"] = prompt_data.get("additionalInstructions", "")
    
    processor = ChunkedProcessor(config)
    ai_processor = GeminiProcessor(config)
    
    return processor.process(transcript_text, ai_processor, mock_mode=mock_mode, output_dir=output_dir, config=config)