...
def get_newest_transcript_dir(video_id: str, base_dir: str = "data/transcripts") -> str:
    ...
    timestamp_dirs = [d for d in os.listdir(video_dir) if os.path.isdir(os.path.join(video_dir, d))]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process a YouTube transcript using AI")
    ...
    parser.add_argument("--model", default="gemini-2.0-flash-lite", help="Override the model specified in the config (default: gemini-2.0-flash-lite)")
    ...
    try:
        ...
        print(f"  Is large transcript: {'Yes' if result.get('large_transcript', False) else 'No'}")
        print(f"  Processing method: {result.get('processing_method', 'standard')}")
        
        # Print file paths
        processed_file = os.path.join(transcript_dir, "processed", "narrative_transcript.txt")
